...
chroot ./rootfs/ qemu-aarch64-static /bin/bash -c 'export PATH="$PATH:/usr/bin:/usr/sbin:/bin:/sbin" && rm -f /etc/systemd/system/dbus-org.bluez.service && systemctl mask systemd-resolved systemd-timesyncd upower bluetooth systemd-journald && dpkg -i /*.deb && rm /*.deb /deb -rf && systemctl enable apt-fix samsung-hwc epoch altresolv upoweralt bluetoothalt batman && apt-mark hold phosh && apt-mark hold phoc && apt-mark hold libwlroots7c && apt-mark hold libx11-xcb1'
...
wget -nv https://github.com/Sexynos/droidian-kernel-samsung-exynos9810/releases/download/star2lte/boot-star2lte.img
wget -nv https://github.com/Sexynos/droidian-kernel-samsung-exynos9810/releases/download/star2lte/recovery.img
...
mkdir out/ -p
DATE=$(date +'%Y%m%d')
ZIP_NAME="droidian-UNOFFICIAL-phosh-phone-samsung_star2lte-api29-arm64-nightly_$DATE.zip"
